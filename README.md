...
5. 点击 **选择文件并发送** 发送单个文件。文字最大 64 KiB，文件最大 2 GiB。大文件先计算 SHA-256，再流式传输；进度条在开始发送后更新。
6. 在 **最近接收** 查看记录、复制文字，或点击 **打开接收文件夹**。应用不会自动运行文件，也不会自动覆盖剪贴板。
...
- 检查 VPN 和防火墙。无需配置路由器端口转发，不建议暴露到公网。
...
- 网络层使用 Python 标准库 `socket`、`ssl`、`threading`，避免额外服务器框架；仅证书生成依赖 `cryptography`。
- TCP 连接首先完成 TLS，校验配对码或已保存的 SHA-256 证书指纹，之后才能发送秘密或内容。
- 消息为 4 字节大端长度 + UTF-8 JSON，版本为 `1`。文件使用 JSON 元信息 → ready 确认 → 原始字节流 → 最终确认；SHA-256 必须匹配后才记录成功。
